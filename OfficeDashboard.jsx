...
const api = {
  ...
  async getMyAgency(existingSession) {
    ...
    // محاولة أولى: جلب الحساب والوكالة في طلب واحد فقط (بدل طلبين متتاليين)
    // عبر العلاقة الخارجية بين profiles و agencies — هذا وحده يقتصّ رحلة
    // شبكة كاملة من وقت أول تحميل للوحة التحكم. إن لم تكن هذه العلاقة
    // معرّفة في قاعدة البيانات نعود تلقائيًا للطريقة القديمة الآمنة.
    const embedded = await supabase
      .from("profiles")
      .select("id, agency_id, full_name, email, role, agencies(*)")
      .eq("id", session.user.id)
      .single();

    if (!embedded.error && embedded.data && embedded.data.agencies) {
      const { agencies, ...profile } = embedded.data;
      return { profile, agency: agencies };
    }
  ...
  async getAgencyCars(agencyId) {
    ...
    // محاولة أولى: تشغيل جلب السيارات وجلب الحجوزات النشطة في نفس الوقت
    // (بالتوازي) بدل الانتظار لانتهاء الأولى قبل بدء الثانية — هذا وحده
    // يقصّ زمن التحميل تقريبًا إلى النصف عند فتح قسم "السيارات".
    const [carsRes, activeRes] = await Promise.all([
      supabase.from("cars").select("*").eq("agency_id", agencyId).order("created_at", { ascending: false }),
      supabase
        .from("bookings")
        .select("car_id, cars!inner(agency_id)")
    ...
    if (!activeRes.error && activeRes.data) {
      activeCarIds = new Set(activeRes.data.map((b) => b.car_id));
    } else {
      // العلاقة الخارجية بين bookings و cars غير معرّفة بالاسم المتوقع —
      // نعود للطريقة الأصلية الآمنة (طلب إضافي بعد معرفة السيارات).
...
const DICTS = {
  ar: {
    ...
    phone_houssem: "هاتف Houssem",
    phone_mahrez: "هاتف Mahrez",
    whatsapp: "رقم واتساب",
    address: "العنوان",
  ...
  fr: {
    ...
    phone_houssem: "Téléphone Houssem",
    phone_mahrez: "Téléphone Mahrez",
  ...
  en: {
    ...
    logo_url: "Logo URL",
    phone_houssem: "Houssem's phone",
    phone_mahrez: "Mahrez's phone",
...
export default function OfficeApp() {
  ...
  const loadContext = useCallback(async (existingSession) => {
    try {
      const ctx = await withTimeout(api.getMyAgency(existingSession), 20000, "بيانات الوكالة");
      setAgencyCtx(ctx);
      setCtxError("");
    } catch (e) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const s = await withTimeout(api.getSession(), 20000, "الجلسة");
        if (cancelled) return;
        if (s) {
          setSession(s);
          await loadContext(s);
      ...
      <div className="page-head">
        ...
        <button className="btn add-car-btn" onClick={openAdd}>
          ...
          <span className="add-car-btn-content">
            <Sparkle size={16} className="add-car-icon" /> {t.add_car}
          </span>
        </button>
      </div>
    ...
    <div className="table-wrap">
      <table className="data-table">
        ...
        <tbody>
          {bookings.map((b) => (
            <tr key={b.id}>
              <td data-label={t.customer_name}>{b.customer_name}</td>
              <td data-label={t.phone} dir="ltr">{b.phone}</td>
              <td data-label={t.car}>{b.cars?.name || "-"}</td>
              <td data-label={t.start_date}>{b.start_date}</td>
              <td data-label={t.end_date}>{b.end_date}</td>
              <td data-label={t.days}>{daysBetween(b.start_date, b.end_date)}</td>
              <td data-label={t.total_price}>{Number(b.total_price).toLocaleString("en-US")} {t.dzd}</td>
...
function SettingsPage({ t, agency, onSaved }) {
  const [form, setForm] = useState({
    ...
    phone_houssem: agency.phone_houssem || "",
    phone_mahrez: agency.phone_mahrez || "",
    ...
    <div>
      ...
      <form className="settings-form" onSubmit={handleSubmit}>
        <div className="panel">
          ...
          <div className="field-row">
            <label className="field">
              <span>{t.phone_houssem}</span>
              <input dir="ltr" value={form.phone_houssem} onChange={(e) => setForm({ ...form, phone_houssem: e.target.value })} />
            </label>
            <label className="field">
              <span>{t.phone_mahrez}</span>
              <input dir="ltr" value={form.phone_mahrez} onChange={(e) => setForm({ ...form, phone_mahrez: e.target.value })} />
            </label>
          </div>
          <div className="field-row">
            <label className="field">
...
function OfficeStyles() {
  return (
    <style>{`
      ...
      .office-root{ background:var(--bg); color:var(--text); font-family:'Tajawal',sans-serif; min-height:100vh; position:relative; isolation:isolate; overflow-x:hidden; }
      ...
      /* الأيقونة الجديدة (نجمة/بريق) بدل علامة +: تبقى في مكانها تمامًا
         ولا تغيّر أي شيء آخر في الزر، فقط تنبض وتدور ببطء بشكل تلقائي. */
      .add-car-icon{ animation:addCarIconSpin 3.2s ease-in-out infinite; transform-origin:center; }
      @keyframes addCarIconSpin{
        0%{ transform:rotate(0deg) scale(1); }
        50%{ transform:rotate(180deg) scale(1.15); }
        100%{ transform:rotate(360deg) scale(1); }
      }
      @media (prefers-reduced-motion: reduce){ .add-car-icon{ animation:none; } }
      ...
      /* على الهاتف (عمودي أو أفقي) الجدول العريض لا يتّسع أبدًا داخل إطار
         الشاشة ويصعب الوصول لعمود الإجراءات دون تمرير أفقي طويل. بدل ذلك،
         تحت هذا العرض يتحوّل كل صف إلى "بطاقة" عمودية تعرض كل معلومة مع
         اسمها، فيبقى كل شيء داخل عرض الشاشة تمامًا بدون أي تمرير جانبي. */
      @media (max-width:760px){
        ...
        .data-table tr{
          ...
          padding:6px 12px; background:var(--card,#161616);
        }
        .data-table td{
          display:flex; align-items:center; justify-content:space-between; gap:12px;
          padding:10px 2px; border-bottom:1px dashed rgba(255,255,255,0.08);
          white-space:normal; text-align:end;
        }
        .data-table td:last-child{ border-bottom:none; }
        .data-table td::before{
          content:attr(data-label); font-weight:700; color:var(--text-sec);
          font-size:0.72rem; text-align:start; flex-shrink:0;
        }
        .data-table td .table-actions{ justify-content:flex-end; width:100%; }
        .data-table td .confirm-row{ justify-content:flex-end; width:100%; flex-wrap:wrap; }
      }
      ...
      /* إطار LED متحرك حول القالب النشط — نفس فكرة أزرار الإضافة، لكن
         بشكل حلقة دوّارة تناسب الشكل الدائري لهذه الأزرار (chips). */
      .filter-chip.active::before{
        content:""; position:absolute; inset:-3px; border-radius:999px; z-index:-1;
        background:conic-gradient(from 0deg, rgba(31,207,160,0) 0%, #1FCFA0 12%, #E8FFF6 20%, #1FCFA0 28%, rgba(31,207,160,0) 42%);
        animation:filterChipLed 2.6s linear infinite;
      }
      @keyframes filterChipLed{ to{ transform:rotate(360deg); } }
      @media (prefers-reduced-motion: reduce){ .filter-chip.active::before{ animation:none; } }
